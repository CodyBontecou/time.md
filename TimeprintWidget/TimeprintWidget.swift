...
/// Timeprint Home Screen Widget
/// Displays screen time data synced from Mac via iCloud
@main
struct TimeprintWidgetBundle: WidgetBundle {
...
struct TimeprintWidget: Widget {
    let kind: String = "TimeprintWidget"
    
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: TimeprintTimelineProvider()) { entry in
            TimeprintWidgetEntryView(entry: entry)
...
struct TimeprintEntry: TimelineEntry {
    let date: Date
    ...
    static var placeholder: TimeprintEntry {
        TimeprintEntry(
            date: Date(),
            todayTotal: 7200,
            weekTotal: 25200,
    ...
    static var empty: TimeprintEntry {
        TimeprintEntry(
            date: Date(),
            todayTotal: 0,
            weekTotal: 0,
...
struct TimeprintTimelineProvider: TimelineProvider {
    
    func placeholder(in context: Context) -> TimeprintEntry {
        .placeholder
    }
    
    func getSnapshot(in context: Context, completion: @escaping (TimeprintEntry) -> Void) {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<TimeprintEntry>) -> Void) {
        let entry = loadEntry()
    ...
    private func loadEntry() -> TimeprintEntry {
...
struct TimeprintWidgetEntryView: View {
    @Environment(\.widgetFamily) var family
    var entry: TimeprintEntry
